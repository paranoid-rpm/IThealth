...
type Protocol = {
  ...
  title: string;
  tags: string[];
  duration: '2 min' | '5 min' | '10 min' | 'Weekly' | '2 weeks';
  steps: string[];
  why: string;
  redFlags?: string[];
};
...
const PROTOCOLS: Protocol[] = [
  ...
  {
    id: 'cvs-microbreaks',
    title: 'CVS: микропауы на 2 минуты (каждый час)',
  ...
  {
    id: 'burnout-2w',
    title: 'Выгорание: протокол стабилизации на 2 недели',
    tags: ['Burnout', 'Сон', 'Стресс'],
    duration: '2 weeks',
    ...
    <div className="glass-card rounded-3xl p-8">
      ...
      {p.redFlags && p.redFlags.length > 0 && (
        <div className="mt-6 rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
          <div className="text-xs font-black tracking-wider uppercase text-red-300">Red flags</div>
...
export default function Tools() {
  ...
  return (
    <div className="container mx-auto px-6 py-16 md:py-24 space-y-8">
      <ContentBlock label="Инструменты" title="Поиск по протоколам + фильтры">
        <p className="text-sm text-muted">
          Поиск по протоколам и быстрые фильтры. Идея страницы: открыть, выбрать тему, и сразу выполнять микро‑план (2–5 минут) в течение дня.
        </p>
